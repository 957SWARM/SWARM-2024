...
// much of this code repackaged from the Choreo source tree, under its original BSD-3 license
...
public class SwarmChoreo {
     ...
     * @param rotationController A generic controller for robot angle, where the first argument is
     *     the measured angle in radians, the second argument is the angle setpoint i
     ...
     *     radians, output: rad/s). This controller will have its continuous input range set to
     *     -pi..pi by ChoreoLib.
     * @param outputChassisSpeeds A function that consumes the target robot-relative chassis speeds
     *     and commands them to the robot.
     * @param useAllianceColor Whether or not to mirror the path based on alliance (this assumes the
     ...
     *     ChoreoCommands.choreoSwerveController(PIDController xController, PIDController
     *     yController, PIDController rotationController) to create one using PID controllers for
     *     each degree of freedom. You can also pass in a function with the signature (Pose2d
     *     currentPose, ChoreoTrajectoryState referenceState) -> ChassisSpeeds to implement a custom
     *     follower (i.e. for logging).
     * @param outputChassisSpeeds A function that consumes the target robot-relative chassis speeds
     *     and commands them to the robot.
     * @param useAllianceColor Whether or not to mirror the path based on alliance (this assumes the
     *     path is created for the blue alliance)
        ...
        return new FunctionalCommand(
                ...
                () -> {
                    ...
                    outputChassisSpeeds.accept(
                            controller.apply(
                                    poseSupplier.get(), trajectory.sample(timer.get(), mirror)));
                },
                (interrupted) -> {
                    timer.stop();
                    if (interrupted) {
     ...
     *     meters, and the output is Y velocity in m/s.
     * @param rotationController A generic controller for robot angle, where the first argument is
     *     the measured angle in radians, the second argument is the angle setpoint in radians, and
     *     the output is angular velocity in rad/s. Keep in mind angular wrapping!
